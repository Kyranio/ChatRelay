...
It's a relay, for language model chat services.

Grant yourself the ease of using any LLM to work with you, inside the IDE of your choice.


> ⚠️ **Currently only supports Visual Studio 2022 and 2026.** 
Other IDEs may be supported in the future, to make that go faster you can help by contributing!

## Why

Some IDEs don't have built-in LLM integrations, which is honestly a good thing for the most part.
However, I recently discovered that these new tools could actually come in useful more than I first would have thought.
In my case, I was using Visual Studio as that is what ~~my employer pays for~~ I use at work. 
So when me and my team were granted to use Claude Code, I quickly discovered that there wasn't an extension for Visual Studio to use it with (that would work with my personal taste).

So, I asked Claude to generate one for me.

After a "few" iterations and tweaks... this is the result. Or at least for now, as I intend to keep improving it and adding features ~~for as long as my employer still pays for claude code~~.
It gives you:
- A dockable chat window inside VS, which relays the messages to Claude CLI or Ollama.
- Permission request bubbles
- MCP configuration management windows
- It's pretty much the Github CoPilot extension window but with a bit more freedom.
